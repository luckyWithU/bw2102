...
pub struct AccountItem {
  ...
  init(itemID: UInt64, typeID: UInt64, owner: Address, metadata: {String:String}) {
    self.itemID = itemID
    self.typeID = typeID
    self.owner = owner
...
pub fun fetch(address: Address, id: UInt64): AccountItem? {
  if let col = getAccount(address).getCapability<&BWItems.Collection{NonFungibleToken.CollectionPublic, BWItems.BWItemsCollectionPublic}>(BWItems.CollectionPublicPath).borrow() {
    if let item = col.borrowBWItem(id: id) {
      return AccountItem(itemID: id, typeID: item.typeID, owner: address, metadata: item.metadata)
    }
  }
